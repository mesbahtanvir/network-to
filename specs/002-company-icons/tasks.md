...
- [X] T012 Add `companyMarks`, `companyMarkData(for:)`, `ensureCompanyMark(_:)` (memory → disk → backend, one in-flight task per reference, failures swallowed), and `clearCompanyMarks()` to `NetworkTo/App/AppStore.swift`; call `clearCompanyMarks()` from `signOut()` and from `deleteAccount()` after the backend confirms (depends on T008–T010)
